...
//    event ApprovalForAll(address indexed _owner, address indexed _operator, bool _approved);
    ...
    function _addLiquidityETH(address tokenA, uint _hours, uint amountADesired, uint amountAMin, uint amountBMin, uint deadline) internal {
        ...
        if(tokenBonus[tokenA][WETHAddress] != 0) {
    ...
    function bonusTokens(address tokenA, address tokenB, uint _hours, uint liquidity) public view returns(uint _bonus){//you have to reverse tokens to get the bonus for each one, only tokenA has bonus here
        ...
        uint bonus = tokensAvailable.mul(_getPercent(_hours, 720)).div(percentage);
        _bonus = bonus.mul(_getPercent(liquidity, poolBalance[tokenA][tokenB])).div(percentage);//get bonus from percent of the pool
    }
    ...
    function _getPercent(uint part, uint whole) private view returns(uint percent) {
        uint numerator = part.mul(percentage);
    ...
    function getPenalty(uint _tokenId) public view returns(uint _penalty){
        uint _percentage = _getPercent(getAge(_tokenId), tokens[_tokenId].maturity).div(percentage.div(100));
    ...
    function getBonus(uint _tokenId) public view returns(uint _bonus){
        uint _percentage = _getPercent(getAge(_tokenId), tokens[_tokenId].maturity).div(percentage);
        uint grossAmount = tokens[_tokenId].LP;
        if(_percentage >= 100) {
            _bonus = penaltyBalance[tokens[_tokenId].tokenA][tokens[_tokenId].tokenB].mul(_getPercent(grossAmount, poolBalance[tokens[_tokenId].tokenA][tokens[_tokenId].tokenB])).div(percentage);//percentage of LP owned and LP owned by this pool multiplied by bonus available
